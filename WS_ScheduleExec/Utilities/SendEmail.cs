        ...
        public static Exception Send_Email(List<string> ToEmail, List<string> CC, string title, string content, bool isHtml)
        {
            ...
            for (int j = 0; j < 5; j++)
            {
                try
                {
                    ...
                    File_Read_Write.Write_File(DateTime.Now.ToString() + ": Sending email \"" + title + "\" TO " + String.Join(", ", ToEmail.ToArray()) + " and " + ((CC == null) ? "no CC" : "CC to " + String.Join(", ", CC.ToArray())) + " Succufully!", true);

                    break;
                }
                catch (Exception er)
                {
                    File_Read_Write.Write_File(DateTime.Now.ToString() + ": Try " + (j + 1) + ": Error - Sending email \"" + title + "\" TO " + String.Join(", ", ToEmail.ToArray()) + " and " + ((CC == null) ? "no CC" : "CC to " + String.Join(", ", CC.ToArray())) + " error! " + er.Message, true);
                }
            }
            return null;
        }
